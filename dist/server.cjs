...
function loadJson(filename, fallback) {
  try {
    const raw = import_fs.default.readFileSync(import_path.default.join(DATA_DIR, filename), "utf-8");
...
function saveJson(filename, data) {
  try {
    import_fs.default.writeFileSync(import_path.default.join(DATA_DIR, filename), JSON.stringify(data, null, 2), "utf-8");
  } catch (e) {
...
var flightsData = loadJson("flights.json", []);
var safetyZones = loadJson("safety-zones.json", []);
var localGuides = loadJson("guides.json", []);
var chatMessages = loadJson("chats.json", []);
var memberPosts = loadJson("social-feed.json", []);
var itemsDatabase = loadJson("marketplace.json", []);
var userBookings = loadJson("bookings.json", []);
var userBucketList = loadJson("bucket-list.json", []);
var photoAlbumExports = loadJson("album-exports.json", []);
var photoAlbumOrders = loadJson("album-orders.json", []);
var contactMessages = loadJson("contacts.json", []);
...
var esc = (v) => String(v ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
app.get("/admin/orders", (req, res) => {
  if (!requireAdmin(req, res)) return;
  const ordersHtml = photoAlbumOrders.length === 0 ? `<tr><td colspan="7" style="text-align:center;color:#94a3b8;padding:2rem">No orders yet</td></tr>` : photoAlbumOrders.map((o) => `
        <tr>
          <td>${esc(o.id)}</td>
          <td>${esc(o.title)}</td>
  ...
  const contactsHtml = contactMessages.length === 0 ? `<tr><td colspan="5" style="text-align:center;color:#94a3b8;padding:2rem">No messages yet</td></tr>` : contactMessages.map((m) => `
        <tr>
          <td>${esc(m.name)}</td>
          <td>${esc(m.email)}</td>
          <td>${esc(m.topic)}</td>
  ...
  const bookingsHtml = userBookings.length === 0 ? `<tr><td colspan="4" style="text-align:center;color:#94a3b8;padding:2rem">No bookings yet</td></tr>` : userBookings.map((b) => `
        <tr>
          <td>${esc(b.id)}</td>
...
<body>
  ...
  <p class="sub">Loaded at ${(/* @__PURE__ */ new Date()).toLocaleString()} \xB7 <a href="?key=${esc(req.query.key)}">Refresh</a></p>
...
app.post("/api/marketplace/bookings", (req, res) => {
  ...
  const newBooking = {
    ...
    addressDetails: isPhysicalPostcard ? { recipient: postalRecipient, street: postalStreet, cityZip: postalCityZip, country: postalCountry, message: postalMessage } : void 0
  };
...
app.post("/api/contact", (req, res) => {
  ...
  const entry = { id: `cm_${Date.now()}`, name, email, topic: topic || "General", message, receivedAt: (/* @__PURE__ */ new Date()).toISOString() };
  contactMessages.unshift(entry);
  saveJson("contacts.json", contactMessages);
  console.log(`[contact] New message from ${name} <${email}> \u2014 ${topic}`);
  sendAdminEmail(
    `\u{1F4EC} New Contact Message \u2014 ${topic || "General"}`,
...
app.get("/api/photo-albums/:id/export.html", (req, res) => {
  ...
  const pages = album.photos.map((photo, index) => `
    <section class="page">
      <p class="kicker">Page ${index + 1}${photo.location ? ` \xB7 ${esc(photo.location)}` : ""}</p>
...
app.post("/api/photo-albums/physical-orders", async (req, res) => {
  ...
  sendAdminEmail(
    `\u{1F4E6} New Print Order \u2014 "${esc(title)}"`,
